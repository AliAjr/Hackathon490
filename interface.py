...
# Crash Hotspot Planner - User Interface

import streamlit as st #Python framework for turning scripts into interaction Web apps
import pandas as pd
import numpy as np
import pydeck as pdk # high performance WebGL mapping and 3D visualization library
...
if uploaded_file is not None:
    ...
    st.warning("Please upload the predictions.csv file generated by Crash_hotspot_prediction.py")
    st.stop()
...
# Precompute Risk, Effectiveness, Reason
...
# Base layer: all candidates (red)
base_layer = pdk.Layer(
    "ScatterplotLayer",
    data=df,
    get_position='[Start_Lng, Start_Lat]',
    get_radius=80,
    get_fill_color="[200, 30, 30, 160]",  # red
    pickable=True,
)

# Highlight layer: selected interventions (green)
...
if not selected.empty:
    highlight_layer = pdk.Layer(
        ...
        get_position='[Start_Lng, Start_Lat]',
        get_radius=150,
        get_fill_color="[0, 200, 50, 220]",  # green
        pickable=True,
    )
